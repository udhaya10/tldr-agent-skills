...
\| Scope \| Path \| Purpose \|
\| \-\-\-\-\-\-\- \| \-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\- \| \-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\- \|
\| Project \| \`/./skills/\` \| Your client's native location \|
\| Project \| \`/.agents/skills/\` \| Cross-client interoperability \|
\| User \| \`~/./skills/\` \| Your client's native location \|
...
\\* Skip directories that won't contain skills, such as \`.git/\` and \`node\_modules/\`
...
Within the same scope (e.g., two skills named \`code-review\` found under both \`/.agents/skills/\` and \`/./skills/\`), either first-found or last-found is acceptable — pick one and be consistent. Log a warning when a collision occurs so the user knows a skill was shadowed.
...
\`\`\`xml theme={null}
pdf-processingExtract PDF text, fill forms, merge files. Use when handling PDFs./home/user/.agents/skills/pdf-processing/SKILL.mddata-analysisAnalyze datasets, generate charts, and create summary reports./home/user/project/.agents/skills/data-analysis/SKILL.md
\`\`\`
...
\`\`\`
The following skills provide specialized instructions for specific tasks.
When a task matches a skill's description, call the activate\_skill tool
...
If no skills are discovered, omit the catalog and behavioral instructions entirely. Don't show an empty \`\` block or register a skill tool with no valid options — this would confuse the model.
...
\*\*Dedicated tool activation\*\*: Register a tool (e.g., \`activate\_skill\`) that takes a skill name and returns the content. This is required when the model can't read files directly, and optional (but useful) even when it can. Advantages over raw file reads:
...
scripts/extract.pyscripts/merge.pyreferences/pdf-spec-summary.md
\`\`\`
